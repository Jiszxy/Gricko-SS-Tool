        ...
        <Grid Margin="18">
            ...
            <Grid Grid.Row="0" Name="TitleBarGrid" Background="Transparent">
                <StackPanel Orientation="Horizontal" HorizontalAlignment="Right" VerticalAlignment="Center">
                    <Button Name="BtnMin" Content="â€”" Width="30" Height="24" Background="#1A1C24" Foreground="#94A3B8" BorderThickness="0" Cursor="Hand" Margin="0,0,5,0">
                        <Button.Resources>
                    ...
                    <Button Name="BtnClose" Content="âś•" Width="30" Height="24" Background="#1A1C24" Foreground="#94A3B8" BorderThickness="0" Cursor="Hand">
                        <Button.Resources>
                            <Style TargetType="Border">
                                <Setter Property="CornerRadius" Value="4"/>
                            </Style>
            ...
            <Grid Grid.Row="1" VerticalAlignment="Center">
                ...
                <StackPanel Name="HomeView" Visibility="Visible" HorizontalAlignment="Center" VerticalAlignment="Center" Margin="0,5,0,0">
                    <Image Name="LogoImgHome" Width="140" Height="68" HorizontalAlignment="Center" Margin="0,0,0,10" RenderOptions.BitmapScalingMode="HighQuality"/>
                ...
                <!-- VIEW 2: 2-MINUTE DEEP SCANNING PROGRESS BAR -->
                <StackPanel Name="ProgressView" Visibility="Collapsed" HorizontalAlignment="Center" VerticalAlignment="Center" Width="420" Margin="0,10,0,0">
                    <Image Name="LogoImgProgress" Width="140" Height="68" HorizontalAlignment="Center" Margin="0,0,0,10" RenderOptions.BitmapScalingMode="HighQuality"/>
                    ...
                    <!-- Status Text e.g. "Scanning memory... â€˘ 50%" -->
                    <TextBlock Name="TxtProgressStatus" Text="Initializing deep PC inspection... â€˘ 0%" Foreground="#94A3B8" FontSize="12" HorizontalAlignment="Center"/>
                </StackPanel>
                ...
                <StackPanel Name="ResultsView" Visibility="Collapsed" HorizontalAlignment="Center" VerticalAlignment="Center" Width="460">
                    ...
                    <Border Name="DetectionBox" Background="#161822" CornerRadius="8" BorderBrush="#25293A" BorderThickness="1" Padding="14,8" Margin="0,0,0,14">
                        <StackPanel HorizontalAlignment="Center">
                            <TextBlock Name="TxtDetectionsBadge" Text="âś” No Cheats or Suspicious Clients Detected" Foreground="#34D399" FontSize="12" FontWeight="Bold" HorizontalAlignment="Center"/>
                ...
                <Grid Name="DetailsView" Visibility="Collapsed" Height="350" Margin="4,0">
                    ...
                    <DockPanel Grid.Row="0" Margin="0,0,0,8">
                        ...
                        <Button Name="BtnBackFromDetails" Content="â† Back" Background="Transparent" Foreground="#38BDF8" BorderThickness="0" FontSize="12" FontWeight="SemiBold" Cursor="Hand" HorizontalAlignment="Right"/>
                    </DockPanel>
...
"@
    ...
    $logoHome           = $window.FindName("LogoImgHome")
    $logoProgress       = $window.FindName("LogoImgProgress")
    $logoResults        = $window.FindName("LogoImgResults")

    # Load Fishbone Logo
    $logoBmp = Get-LogoSource
    if ($logoBmp) {
        $logoHome.Source = $logoBmp
        $logoProgress.Source = $logoBmp
        $logoResults.Source = $logoBmp
    }

    # Enable Free Window Dragging From Anywhere
    $dragHandler = {
        param($s, $e)
    ...
    $rootBorder.Add_MouseLeftButtonDown($dragHandler)
    $titleBarGrid.Add_MouseLeftButtonDown($dragHandler)
    ...
    function Add-CleanDetailLine {
        ...
        $tbCat.FontWeight = [System.Windows.FontWeights]::SemiBold
        $sp.Children.Add($tbCat) | Out-Null

        $tbStat = [System.Windows.Controls.TextBlock]::new()
        $tbStat.Text = "$Status "
        $statColor = switch ($Status) {
            "[FLAG]" { "#EF4444" }
            "[WARN]" { "#F59E0B" }
            "[CLEAN]" { "#10B981" }
            default { "#38BDF8" }
        }
        $tbStat.Foreground = [System.Windows.Media.BrushConverter]::new().ConvertFromString($statColor)
    ...
    $btnScan.Add_Click({
        ...
        for ($pct = 1; $pct -le 15; $pct++) {
            ...
            $txtProgressStatus.Text = "Scanning active memory & running processes... â€˘ $pct%"
            Pump-WpfEvents
        ...
        for ($pct = 16; $pct -le 35; $pct++) {
            $scanProgress.Value = $pct
            $txtProgressStatus.Text = "Deep scanning Minecraft instances, versions & mods... â€˘ $pct%"
            Pump-WpfEvents
            Start-Sleep -Milliseconds 1200
        }
        ...
        for ($pct = 36; $pct -le 60; $pct++) {
            $scanProgress.Value = $pct
            $txtProgressStatus.Text = "Scanning Windows Prefetch & BAM kernel timestamps... â€˘ $pct%"
            Pump-WpfEvents
            Start-Sleep -Milliseconds 1200
        }
        ...
        for ($pct = 61; $pct -le 80; $pct++) {
            $scanProgress.Value = $pct
            $txtProgressStatus.Text = "Auditing UserAssist ROT13 & execution traces... â€˘ $pct%"
            Pump-WpfEvents
            Start-Sleep -Milliseconds 1200
        }
        Scan-UserAssist
        ...
        for ($pct = 81; $pct -le 95; $pct++) {
            $scanProgress.Value = $pct
            $txtProgressStatus.Text = "Auditing file systems, temp drops & anti-forensics... â€˘ $pct%"
            Pump-WpfEvents
            Start-Sleep -Milliseconds 1200
        }
        ...
        for ($pct = 96; $pct -le 100; $pct++) {
            $scanProgress.Value = $pct
            $txtProgressStatus.Text = "Finalizing forensic report & scorecard... â€˘ $pct%"
            Pump-WpfEvents
            Start-Sleep -Milliseconds 1200
        }
        ...
        if ($inst -and $inst.LastPlayedTime) {
            ...
            $txtResultClient.Text = "Client   : $($inst.LauncherName)"
        ...
        if ($actualCheats.Count -gt 0) {
            ...
            $txtDetectionsBadge.Text = "âš  SUSPICIOUS CLIENT / CHEATS DETECTED"
            ...
            $txtDetectionsBadge.Text = "âś” No Cheats or Suspicious Clients Detected"
